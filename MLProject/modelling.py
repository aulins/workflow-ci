...
def train(data_path: Path):
    ...
    # 3) Beri sedikit jeda untuk memastikan filesystem siap
    time.sleep(1)

    # 4) Set MLflow lokal (folder mlruns akan dibuat otomatis)
    mlflow_tracking_path = os.path.abspath("./mlruns").replace("\\", "/")
    ...
    # 6) Train + log manual untuk metrik utama (agar jelas di UI)
    with mlflow.start_run(run_name="logreg_baseline"):
        ...
        # 7) Log artefak: confusion matrix
